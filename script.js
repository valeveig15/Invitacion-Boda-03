(() => {
  ...
  // Autoplay suave de la galería. Se pausa al interactuar para no pelear con el usuario.
  const carousel = qs('.carousel');
  const prefersReducedMotion = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches;
  let carouselTimer = null;
  function startCarouselAuto(){
    if(prefersReducedMotion || !slideImg || carouselTimer) return;
    carouselTimer = setInterval(()=>{ slideIndex=(slideIndex+1)%slides.length; renderSlide(); }, 5200);
  }
  ...
  carousel?.addEventListener('touchstart',stopCarouselAuto,{passive:true});
  carousel?.addEventListener('touchend',()=>setTimeout(startCarouselAuto,2200),{passive:true});
  document.addEventListener('visibilitychange',()=>document.hidden?stopCarouselAuto():startCarouselAuto());
  startCarouselAuto();
